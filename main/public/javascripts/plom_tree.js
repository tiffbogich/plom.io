...
function plom_tree(selector, data, callback) {
  ...
  function update(source) {
    ...
    var nodeEnter = node.enter().append("g")
      .attr("class", "node")
      .attr("id", function(d){return '_id' + d._id;}) //CSS identifiers can't start with a number
  ...
  return function(newnode, node){
    ...
    var newnodes = tree.nodes(newnode).reverse();

    if('children' in node){
      node.children.push(newnodes[0]);
    } else {
